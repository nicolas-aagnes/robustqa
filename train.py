...
def read_and_process(args, tokenizer, dataset_dict, dir_name, dataset_name, split):
    ...
    if os.path.exists(cache_path) and not args.recompute_features:
        print(f"Loaded tokenized sampels from {cache_path}")
        ...
        print(f"Saving tokenized samples to {cache_path}")
        ...
        for epoch_num in range(self.num_epochs):
            ...
            with torch.enable_grad(), tqdm(
                ...
                for batch in train_dataloader:
                    ...
                    # Output tokens are [start, end], unless no answer then it's [0, 0].
                    loss = outputs[0]
                    loss.backward()
                    optim.step()
...
def main():
    ...
    if args.finetune_path is not None:
        model = DistilBertForQuestionAnswering.from_pretrained(args.finetune_path)
    else:
    ...
    if args.do_train:
        ...
        best_scores = trainer.train(
            model, train_loader, val_loader, val_dict, tokenizer
        )
    if args.do_eval:
        args.device = (
            torch.device("cuda") if torch.cuda.is_available() else torch.device("cpu")
